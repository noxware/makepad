use {
    crate::makepad_draw::*,
    std::{any::TypeId, cell::{RefCell, Ref, RefMut}, collections::BTreeMap, fmt::{self, Debug, Error, Formatter}, rc::Rc}
};
...
impl LiveApply for WidgetSet {
    fn apply(&mut self, cx: &mut Cx, apply: &mut Apply, index: usize, nodes: &[LiveNode]) -> usize {
        for inner in self.iter() {
            if !inner.is_empty() {
                return inner.0.borrow_mut().apply(cx, apply, index, nodes);
            }
        }
...
impl WidgetRef {
    // #[deprecated]
    pub fn empty() -> Self {
        Self (Rc::new(RefCell::new(Box::new(EmptyWidget))))
    }
    
    // #[deprecated]
    pub fn is_empty(&self) -> bool {
        RefCell::borrow(&self.0).downcast_ref::<EmptyWidget>().is_some()
    }
    ...
    fn apply(&self, cx: &mut Cx, apply: &mut Apply, index: usize, nodes: &[LiveNode]) -> usize {
        if let LiveValue::Class {live_type, ..} = nodes[index].value {
            if !self.is_empty() {
                if RefCell::borrow(&self.0).ref_cast_type_id() != live_type {
                    *self.0.borrow_mut() = Box::new(EmptyWidget); // type changed, drop old component
                    log!("TYPECHANGE {:?}", nodes[index]);
                } else {
                    return RefCell::borrow_mut(&self.0).apply(cx, apply, index, nodes);
                }
            }
            if let Some(component) = cx.live_registry.clone().borrow().components.get::<WidgetRegistry>().new(cx, live_type) {
                if cx.debug.marker() == 1 {
                    panic!()
                }
                *self.0.borrow_mut() = component;
                if !self.is_empty() {
...
impl WidgetNode for EmptyWidget {
    fn find_widgets(&mut self, _path: &[LiveId], _cached: WidgetCache, _results: &mut WidgetSet) {
    }
...
impl LiveApply for EmptyWidget {
    fn apply(&mut self, _cx: &mut Cx, _apply: &mut Apply, _index: usize, _nodes: &[LiveNode]) -> usize {
        0
    }
}
